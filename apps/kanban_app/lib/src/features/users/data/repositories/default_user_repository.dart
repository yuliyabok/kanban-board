  ...
  Future<Result<List<UserEntity>>> search(String query) async {
    try {
      if (_usesServerRemote && _remoteDataSource != null) {
        final remoteUsers = await _remoteDataSource.search(query);
        for (final user in remoteUsers) {
          await _localDataSource.upsert(user.toEntity().toCompanion());
        }
        return Success(
          remoteUsers.map((user) => user.toEntity()).toList(growable: false),
        );
      }
