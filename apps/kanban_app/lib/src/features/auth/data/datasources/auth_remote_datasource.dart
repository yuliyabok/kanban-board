...
final class LocalAuthRemoteDataSource implements AuthRemoteDataSource {
  ...
  static const _passwordHashKeyLength = 32;
  ...
  bool _verifyPassword({
    ...
    if (expectedHash.startsWith('$_passwordHashAlgorithm\$')) {
      return _hashPassword(password, salt) == expectedHash;
    }
  ...
  List<int> _pbkdf2({
    ...
    while (blocks.length < _passwordHashKeyLength) {
      ...
      for (var i = 1; i < _passwordHashIterations; i++) {
    ...
    return blocks.take(_passwordHashKeyLength).toList(growable: false);
  }
}
