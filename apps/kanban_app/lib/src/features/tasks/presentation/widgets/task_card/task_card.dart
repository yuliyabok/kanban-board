...
class _TaskCardState extends ConsumerState<TaskCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final effectiveBackground = Color.alphaBlend(
      Theme.of(context).colorScheme.surfaceContainerHigh.withValues(
        alpha: widget.task.cardBackgroundColor == null ? 0.18 : 0.08,
      ),
      backgroundColor,
    );
    final showActions =
      ...
      child: MouseRegion(
        ...
        child: AppCard(
          ...
          borderColor: isMainTask
              ...
              : Theme.of(context).colorScheme.outline.withValues(alpha: 0.54),
          borderWidth: isMainTask ? 2 : 1.35,
  ...
  List<Widget> _assigneeAvatars() {
    ...
    if (values.isEmpty) {
      return [TaskAssigneeAvatar(label: widget.task.assigneeName)];
    }
    return [
      for (final assignee in values.take(4))
        _TaskAssigneeAvatarForUser(userId: assignee.userId),
    ];
  }
}

class _TaskAssigneeAvatarForUser extends ConsumerWidget {
  const _TaskAssigneeAvatarForUser({required this.userId});
