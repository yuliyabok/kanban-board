...
class TasksController extends AsyncNotifier<void> {
  ...
  Future<void> toggleComplete({
    ...
    final result = await ref
        .read(taskCommandServiceProvider)
        .toggleComplete(boardId: boardId, taskId: taskId);
    state = _toAsyncValue(result);
  }
  ...
  Future<void> updateTask(TaskEntity task) async {
    ...
    final result = await ref
        .read(taskCommandServiceProvider)
        .updateTask(UpdateTaskCommand(task));
    state = _toAsyncValue(result);
  }
  ...
  Future<void> moveToColumn({
    ...
    final result = await ref
        .read(taskCommandServiceProvider)
        .moveTask(
          MoveTaskCommand(boardId: boardId, task: task, columnId: columnId),
        );
    state = _toAsyncValue(result);
  }
  ...
  Future<void> delete(String taskId) async {
    ...
    final result = await ref
        .read(taskCommandServiceProvider)
        .deleteTask(DeleteTaskCommand(taskId: taskId));
    state = _toAsyncValue(result);
  }
  ...
  Future<void> deleteAndPromoteChildren(String taskId) async {
    ...
    final result = await ref
        .read(taskCommandServiceProvider)
        .deleteTask(DeleteTaskCommand(taskId: taskId, cascade: false));
    state = _toAsyncValue(result);
  }
